...
public class JugadorController {
	...
	public ResponseEntity<?> updateJugador(@RequestBody Jugador j)
		...
		try {
			...
			JugadorDTO jDTO = jugadorMap.convertToDto(jugador);
	...
	@GetMapping(value = "/players/buscar/{nombre}")
	public ResponseEntity<?> buscJugador(@PathVariable String nombre) {
		JugadorDTO jDTO = new JugadorDTO();
		jDTO = jugadorServiceImpl.findByNombre(nombre);

		if (jDTO != null) {
			return new ResponseEntity<JugadorDTO>(jDTO, HttpStatus.OK);

		} else {
			return new ResponseEntity<String>("Error al buscar al jugador con nombre: " + nombre,
